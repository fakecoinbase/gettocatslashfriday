...
class script {
    ...
    static addressToScript(app, addr) {
        return new script([
            'OP_DUP',
            'OP_HASH160',
            ['PUSHDATA', app.btcchain.ADDRESS.getPublicKeyHashByAddress(addr), 14],
            'OP_EQUALVERIFY',
            'OP_CHECKSIG'
        ]).create();
    }
    static isP2PKH(app, buffer) {
        let res = true;
    ...
    static scriptToAddr(app, scriptBuffer) {
        let adrhash = script.scriptToAddrHash(scriptBuffer);
        return app.btcchain.ADDRESS.generateAddressFromAddrHash(adrhash.toString('hex'));
    }
